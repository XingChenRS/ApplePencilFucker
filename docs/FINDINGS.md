...
1. 苹果不是"做不到"，而是在 `accessoryd` 里按**机型白名单**拒绝：初代铅笔
   （附件型号 `A1603`）只有在 `DeviceSupports9Pin`（9 针 Lightning 能力）或
   机型是 `iPad13,18 / iPad13,19`（iPad 10）时才允许走**有线带外配对**。
   USB-C iPad Pro 三者皆非 → 弹"配件不受支持"并跳过配对。
2. 手动用 BLE 工具配对能不落系统钥匙串，所以每次断连都要"忽略设备"重配。
3. 解法：注入 `accessoryd`，只让那段判定用到的两个 MobileGestalt 键
   （`DeviceSupportsApplePencil`、`DeviceSupports9Pin`）返回 true。
   两个键在整个守护进程里各只出现一次，且都不在 MG 磁盘缓存里。
4. 实测结果：插线不再弹窗 → 完成配对 → **断连自动重连**，**重启后（甚至不重新
   越狱）依然直接连上，无需忽略设备**。密钥确实落进了系统钥匙串。
...
最终验证（2026-09-20 用户实测）：
1. 断连后**自动重连** ✓，无需"忽略设备"
2. **重启 iPad 后（未重新越狱）** 仍能直接连接，且在蓝牙里不需要忽略设备重配
   —— 证明链路密钥已持久化在系统钥匙串 ✓
3. 未越狱时插线仍会提示"配件不受支持"（插件只在越狱环境生效），但**已建立的
   配对不受影响** ✓

仍未做（不影响使用）：
- type==25 对应的设备类型枚举名（仅涉及设置页那条 UI 路径）
